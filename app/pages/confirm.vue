  ...
  <v-app>
    <v-container class="confirm-container d-flex align-center justify-center">
      <v-card class="confirm-card text-center" elevation="10">
        <v-progress-circular v-if="loading" indeterminate color="cyan lighten-3" size="60" width="6" class="mb-6" />
        ...
        <p v-if="!loading" class="text-body-2 mb-6">Your EduCode account is now active. Redirecting to login...</p>

        <v-btn v-if="!loading" class="btn-primary" @click="goLogin">Go to Login</v-btn>
      </v-card>
    </v-container>
  </v-app>
</template>
