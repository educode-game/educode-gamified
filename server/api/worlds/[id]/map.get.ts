...
export default defineEventHandler(async (event) => {
  ...
  const id = params?.id
  if (!id) throw createError({ statusCode: 400, message: 'Missing world id' })

  const token = (getRequestHeader(event, 'authorization') || '').replace('Bearer ', '')
  const user = await getUserFromToken(token)
  const client = createServiceSupabase()

  const { data: world } = await client.from('worlds').select('*').eq('id', id).maybeSingle()
  ...
  let progress: any[] = []
  if (user) {
    const { data: up } = await client.from('user_progress').select('*').eq('user_id', user.id)
    progress = up || []
  }
  ...
  return { world, challenges: enriched }
})
